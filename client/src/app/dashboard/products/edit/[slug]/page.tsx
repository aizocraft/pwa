...
import { ArrowLeft, Save, X, Plus, Trash2, ImageIcon, Loader2 } from 'lucide-react'
...
export default function EditProductPage() {
  ...
  const productSlug = params.slug as string  // Changed from 'id' to 'slug'
  ...
  const { data: product, isLoading, error } = useQuery({
    ...
    queryFn: () => getProduct(productSlug),  // Pass slug to getProduct
    enabled: !!productSlug
  })
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        tags: product.tags || [],
        specifications: product.specs || (product as any).specifications || {},
        brand: product.brand || '',
        type: product.type || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // We need the product ID for update, so we must fetch it from the product data
    if (!product?._id) {
